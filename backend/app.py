...
# --- **CORRECCIÓN 1** (Helper para limpiar respuesta de IA) ---
def _limpiar_respuesta_ia(respuesta_bruta):
    # Elimina los fences de markdown (```) y lenguajes (```python, ```css)
    codigo_limpio = re.sub(r"```[a-z]*\n", "", respuesta_bruta)
    codigo_limpio = re.sub(r"```", "", codigo_limpio)
    # Elimina espacios/líneas en blanco al inicio y al final
...
def medir_codigo_con_codecarbon(codigo, codigo_id, tipo_prueba):
    ...
    try:
        ...
        # Check para HTML/CSS (este estaba bien)
        if "<div" in codigo or "<style" in codigo or "<html" in codigo:
            print("Código HTML/CSS detectado. Omitiendo exec().")
            import time
            time.sleep(0.1) 
        else:
            # Solo ejecutar si es Python
            print("Código Python detectado. Ejecutando con exec().")
        ...
        # --- **CORRECCIÓN 2** (Usar camelCase: emisionesCO2 y consumoCPU) ---
...
def optimizar_codigo():
    ...
    # --- IA REAL (OLLAMA) ---
    prompt = f"Eres un experto en optimización. Optimiza el siguiente código para eficiencia energética. Responde *única y exclusivamente* con el bloque de código optimizado. No añadas explicaciones, saludos, ni markdown. CÓDIGO:\n{codigo_original}"
    respuesta_ia_bruta = call_ollama(prompt)
    
    if "# ERROR" in respuesta_ia_bruta:
        return jsonify({'error': respuesta_ia_bruta}), 500

    # --- **CORRECCIÓN 1** (Limpiar la respuesta de la IA) ---
    codigo_optimizado = _limpiar_respuesta_ia(respuesta_ia_bruta)
...
def sugerir_mejoras():
    ...
    # --- IA REAL (OLLAMA) ---
    prompt = f"Eres un analista de código. Dame sugerencias de eficiencia para este código. Responde *única y exclusivamente* con una lista de viñetas ('-'). No añadas saludos, explicaciones ni texto introductorio. CÓDIGO:\n{codigo}"
    respuesta_ia = call_ollama(prompt) # gemma:2b
